...
def preprocess_images(filedir, filenames, lateralities, datafilename):
    ...
    datafile = tables.open_file(datafilename, mode='w') #UInt16Atom to avoid running out of memory. 
...
def crop(dat):
    # argwhere will give you the coordinates of every nonzero point
    true_points = np.argwhere(dat)
    # take the smallest-valued points and use them as the top left of your crop
    top_left = true_points.min(axis=0)
    # take the largest-valued points and use them as the bottom right of your crop
    bottom_right = true_points.max(axis=0)
    # add a 1 because slice isn't inclusive
    return dat[
